...
/*     httpClient.addAuthenticator((Request request) async {
  ...
  // Working
  Future<String?> forgotPasswordVerify(ForgotPasswordVerifyRequest request) {
    print(request.toJson());
    return post<ApiResponse>('/auth/forgot-password-verify', request.toJson())
        .then(
      (value) {
        var response;
  ...
  Future<String> forgotPassword(String identifier) {
    if (false) {
      return Future.delayed(
          Duration(seconds: 1), () => '6003004b-6de2-470d-aae5-df83c2066a7f');
    }

    return post<ApiResponse>(
            '/auth/forgot-password?identifier=$identifier', {'dsdsd': 'fdfdf'})
        .then(
      (value) {
        var response;
        ...
        if (response != null) {
          ...
          response = value.body?.data['pinId'];
        }
...
/*     {
  ...
  Future<String> purchasePlan({
    ...
    if (discount_id != null) request.addAll({'discount_id': discount_id});

    return post<ApiResponse>('/plans/purchase', request).then(
      (value) {
        var response;
