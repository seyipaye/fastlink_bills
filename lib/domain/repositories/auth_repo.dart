...
class AuthRepository {
  ...
  Future<String?> forgotPasswordVerify({
    required ForgotPasswordVerifyRequest request,
  }) async {
    final response =
        await AuthProvider.value.forgotPasswordVerify(request).then((value) {
      // Delete the user
  ...
  Future<String?> forgetPassword({String? identifier}) async {
    final String response;

    user.value = User.forgotPassword(
        ForgotPasswordVerifyRequest(identifier: identifier!));

    response = await AuthProvider.value.forgotPassword(identifier);

    user.value = User.forgotPassword(
        ForgotPasswordVerifyRequest(identifier: identifier, pin_id: response));
  ...
  Future<String?> purchasePlan(
      {required int plan_id, String? discount_id}) async {
